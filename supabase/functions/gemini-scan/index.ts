...
serve(async (req) => {
  ...
  try {
    ...
    const prompt = `
      ...
      - **주의:** 숫자 5와 6, 1과 7 등을 혼동하지 않도록 글자 모양을 극도로 세밀하게 관찰하세요. (예: 25와 26을 헷갈리지 마세요)
      ...
      정확한 JSON 형식으로만 응답해 주세요:
      {
        ...
        "isExpired": false,
        "safetyStatus": "safe",
        "notes": "읽어낸 팩트(브랜드명, 컬러명, 제조번호(LOT), 제조일자, 유통기한 등)만 간략하게 객관적으로 나열하세요. 절대로 만료 여부나 안전성에 대한 주관적 판단(예: '안전합니다', '지나지 않았습니다' 등)을 적지 마세요. 판단은 시스템이 합니다."
      }
